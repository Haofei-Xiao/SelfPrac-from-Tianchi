...
graph = df.groupby(['JOB_TITLE'])['PREVAILING_WAGE'].mean().nlargest(10)
graph = graph.to_frame().reset_index()  # convert series to df for graph

sns.barplot(x=graph["PREVAILING_WAGE"], y=graph["JOB_TITLE"])
plot = sns.barplot(x=graph["PREVAILING_WAGE"], y=graph["JOB_TITLE"])

# adjusing visual effect
plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.3, bottom=0.1, right=0.96, top=0.9)
ax.set_ylabel('Jobs')
ax.set_xlabel('wages (10^7)')
ax.set_title('Avg wage of top 10 jobs applying h1b')

# put value in bar graph
for index, value in enumerate(graph["PREVAILING_WAGE"]):
    plt.text(value, index, s=round(value, 2))

plt.savefig("top 10 h1b job wage.png")
plt.show()